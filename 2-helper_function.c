#include "main.h"

/**
 * _putchar - prints the character given as argument
 * @c: argument to be printed
 *
 * Return: an integer
 */
int _putchar(char c)
{
	return (write(1, &c, 1));
}


/**
 * convert_size - function to convert the size of a numeric value
 * @n: the integer variable
 * @size: the new size
 *
 * Return: an integer
 */
long int convert_size(long int n, int size)
{
	if (size == 2)
		return ((short)n);
	if (size == 1)
		return (n);
	else
		return ((int)n);
}

/**
 * print_int - function to print an integer
 * @args: arguments to print
 * @count: number of bytes to print
 * @size: the integer size
 *
 * Return: an integer
 */

int print_int(va_list args, int *count, int size)
{
	int last, digit, i = 1;
	long int n, num, exp = 1;

	n = va_arg(args, long int);
	n = convert_size(n, size);
	last = n % 10;
	n = n/10;
	num = n;

	if(last < 0)
	{
		_putchar('-');
		num = -num;
		n = -n;
		last = -last;
		i++;
	}
	if(num > 0)
	{
		while (num / 10 != 0)
		{
			exp *= 10;
			num /= 10;
		}
		num = n;
		while (exp > 0)
		{
			digit = num /exp;
			_putchar(digit + '0');
			num -= (digit * exp);
			exp /= 10;
			i++;
		}
	}
_putchar(last + '0');
*count += i;
return (i);
}

/**
 * print_dec - function to print decimal
 * @args: arguments to print
 * @count: number of bytes printed 
 * @size: argument specifying size
 *
 * Return: an integer
 */
int print_dec(va_list args, int *count, int size)
{
	int last, digit;
	long int n, num, exp = 1;
	int i = 1;

	n = va_arg(args, long int);
	n= convert_size(n, size);
	last = n % 10;
	n /= 10;
	num = n;

	if (last < 0)
	{
		_putchar('-');
		num = -num;
		n = -n;
		last = -last;
		i++;
	}
	if (num > 0)
	{
		while (num / 10 != 0)
		{
			exp *= 10;
			num /= 10;
		}
		num = n;
		while (exp > 0)
		{
			digit = num / exp;
			_putchar(digit + '0');
			num -= (digit *exp);
			exp /= 10;
			i++;
		}
	}
_putchar(last + '0');
*count += i;
return(i);
}

/**
 * print_unsigned - function to print an unsigned integer
 * @args: list of arguments
 * @count: number of bytes to be printed
 * @size: argument specifying size
 *
 * Return: an integer
 */
int print_unsigned(va_list args, int *count, int size)
{
	unsigned long int n, num, exp = 1;
	int last, digit;
	int i = 1;

	n = va_arg(args, unsigned long int);
	n = convert_size(n, size);
	last = n % 10;
	n /= 10;
	num = n;

	if (last < 0)
	{
		_putchar('-');
		num = -num;
		n = -n;
		last = -last;
		i++;
	}
	if (num > 0)
	{
		while (num / 10 != 0)
		{
			exp *= 10;
			num /= 10;
		}
		num = n;
		while (exp > 0)
		{
			digit = num /exp;
			_putchar(digit + '0');
			num -= (digit * exp);
			exp /= 10;
			i++;
		}
	}
	_putchar(last + '0');
	*count += i;
	return (i);
}
